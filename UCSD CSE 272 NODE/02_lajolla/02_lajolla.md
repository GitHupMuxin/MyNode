...
![Pasted-image-20260408163458.png](image/Pasted-image-20260408163458.png)
这个内容应该是对光源进行重要性采样，在GAMES101 光线追踪的课程中有详细的讲解。
这边对照了两个采样方式即Next event estimation 和 cosine weighted hemisphere
![Pasted-image-20260408163758.png](image/Pasted-image-20260408163758.png)
在小光源的情况下，NEE的效果会比较好
![Pasted-image-20260408163848.png](image/Pasted-image-20260408163848.png)
当接近光源时，则余弦加权采样的效果会比较好
**原因**
![Pasted-image-20260408164037.png](image/Pasted-image-20260408164037.png)
...
![Pasted-image-20260408165006.png](image/Pasted-image-20260408165006.png)多重重要性采样会中和两种采样方式的优点
![Pasted-image-20260408165153.png](image/Pasted-image-20260408165153.png)
## 为何大多数模型的使用三角形作为图元
### 如何处理光线和三角形的相交
...
![Pasted-image-20260408193831.png](image/Pasted-image-20260408193831.png)
...
**光栅化管线**：在确定 mipmap 层级时需要计算 $\frac{\partial u}{\partial x} , \frac{\partial u}{\partial y}, \frac{\partial v}{\partial x}, \frac{\partial v}{\partial y}$，从而得到 
$$
lob = log2(max(length(\frac{\partial u}{\partial x},\  \frac{\partial v}{\partial x}),\  length(\frac{\partial u}{\partial y},\  \frac{\partial v}{\partial y})))
$$
**光线追踪管线**：
- **光线微分**：这是最通用的方法。它追踪一条主光线及其在屏幕X/Y方向上各偏移一个像素的“邻居光线”。通过计算邻居光线与同一物体表面交点处的UV值差异，就能近似得到`dU/dX`等偏导数，从而对接光栅化的LOD公式。此方法质量高，但计算和存储开销大。
- **光线锥**：将光线视为一个有半径的锥体，而非无限细的线。随着光线传播，锥体半径会根据与相机的距离和表面曲率等因素扩大。当光线击中表面时，锥体的截面积就是采样的足迹，直接用于确定Mipmap层级。这是一种在质量和性能之间取得良好平衡的方法。
![Pasted-image-20260408193803.png](image/Pasted-image-20260408193803.png)
![](image/Pasted%20image%2020260528004820.png)

### 对于三角形来说，其构成的模型肯定是有棱有角的，对于这种情况，可以在三角形里面用重心坐标插值来得到一个较为圆滑的过度
![Pasted-image-20260408193740.png](image/Pasted-image-20260408193740.png)
...
![Pasted-image-20260408193903.png](image/Pasted-image-20260408193903.png)
...
![Pasted-image-20260408194157.png](image/Pasted-image-20260408194157.png)
# 数学
