...
![Pasted-image-20260405204906.png](image/Pasted-image-20260405204906.png)
...
![Pasted-image-20260405205118.png](image/Pasted-image-20260405205118.png)
对于计算机来说，要计算该数值，我们只能采样，这样就变成了一个离散的求和。即**黎曼积分**。
![Pasted-image-20260405205213.png](image/Pasted-image-20260405205213.png)
现在问题就变成了**采样和估计，主要矛盾是：如何正确且快速的去估计该像素的颜色**
![Pasted-image-20260405205357.png](image/Pasted-image-20260405205357.png)
于是就有了**重要性采样**
重要性采样在 games 101 在有详细的介绍一种方式：即在计算直接光照时对光源的采样。
为了**物理的正确性**，我们需要一种方法将积分进行转换，于是运用了**换元积分法**。
于是：
![Pasted-image-20260405205820.png](image/Pasted-image-20260405205820.png)
如果你理解了上述内容后续的估计都能看懂了
你也可以认为，**黎曼积分是一种采样概率为面积分之一的均匀采样的蒙特卡罗积分。**

**三角形滤波将一个原本应该进行均匀采样的黎曼积分变成了一个中间采样概率大，周边采样概率小的一个蒙特卡罗积分。
![Pasted-image-20260405210059.png](image/Pasted-image-20260405210059.png)
然后再结合蒙特卡罗积分**
![Pasted-image-20260405210201.png](image/Pasted-image-20260405210201.png)
## 概率分布的生成
...
![Pasted-image-20260405211814.png](image/Pasted-image-20260405211814.png)
对于多少能量反射多少能量折射，则引入一个折射率，使用Schlick's approximation
![Pasted-image-20260405211901.png](image/Pasted-image-20260405211901.png)
上述为前情提要。
## 代码实现思想：
### 如何得到切线方向
通过数学方式得到切线方向
![Pasted-image-20260408154000.png](image/Pasted-image-20260408154000.png)
以下是另一种推导
![Pasted-image-20260408161215.png](image/Pasted-image-20260408161215.png)
这时候，根据菲尼尔定理，我们可以得到 $\sin\theta_t$ 和对应的 $\cos\theta_t$ ，同时，平面的法向量 $N$ 是已知的。
由于 $L$ 和 $T$ 都是单位向量这时候我们只需要知道 $G$ 方向即可得到折射方向。
对于折射方向，分解成于 $-N$ 方向和 $-G$ 方向的两个向量。对于 $-N$ 方向的向量方向是已知，大小为 $\cos\theta_t$ 。$-G$ 方向的向量方向未知，但大小为 $\sin\theta_t$。这时候 $G$ 方向和 $perp_NL$ 方向相同，
即
$$G = \frac{L - dot(L,N) * N} {\sin\theta_L}$$
这时，$T$ 为两个分解向量的和。
$$T = -N * \cos\theta_t + (- G * \sin\theta_t)$$
即 
$$T =  -N * \cos\theta_t - \frac{L - dot(L,N) * N} {\sin\theta_L} * \sin\theta_t$$
### 如何进行光线的递归选择
**判断是否发生全反射**
根据斯涅耳定律计算 $\cos⁡2\theta_t=1−η2(1−\cos⁡2\theta_i)$
若 $\cos⁡2\theta_t<0$，则无折射光线，**只追踪反射光线**（反射率 $R=1$），直接返回反射贡献。
**计算反射率 $R$ 和透射率 $T$**
...
期望：
$$E(L_i) = R * T_r + (1 - R) * T_t$$
俄罗斯轮盘赌：以概率 $P$ 追踪反射（权重 $\frac{R}{P}$），以概率 $1−P$ 追踪折射（权重 $\frac{T}{1 - P}$）
smallpt 的混合策略：**深度较小时（≤2）使用确定性分裂**，避免早期随机截断造成的噪声；**深度较大时使用俄罗斯轮盘赌**，概率 $P=0.25+0.5R$
这时候需要修改权重分别为：$\frac{R}{P}、\frac{1 - R}{1 - P}$;
这时期望为：
$$E(L_i) = \frac{R}{P} * P * L_r + \frac{1 - R}{1 - P} * (1 - P) * L_t = R * T_r + (1 - R) * T_t$$
...
为了降低方差，我们希望**两个分支被选中的概率不要过于悬殊**。常见的启发式是让概率 $P$ 位于某个中间范围，比如 [0.25, 0.75]。同时，为了保持无偏，需要给选中分支的贡献除以对应的概率（权重补偿）。
